...
function updateContributionRank(players) {
    // 按contribution降序排序
    const sorted = [...players].sort((a, b) => parseFloat(b.contribution) - parseFloat(a.contribution));
...
// 修改点过滤器的事件监听器
document.getElementById('point-filter').addEventListener('change', function(e) {
    currentFilters.point = e.target.value;
    applyFilters();
});

// 修改贡献度过滤器的事件监听器
document.getElementById('min-contribution').addEventListener('change', function(e) {
    currentFilters.minContribution = parseFloat(e.target.value);
    applyFilters();
});

document.getElementById('max-contribution').addEventListener('change', function(e) {
...
const latitudeRanges = {
    ...
    'MF': { start: -20, end: 0 },      // 南纬0-30度
    'DF': { start: 0, end: 20 },       // 北纬0-30度
    'GK': { start: 20, end: 60 }       // 北纬30-60度
};
const colors = {
    'FW': {
        base: '#FFB6C1', // 浅粉色
    ...
    if (currentTeam === 'ALL') {
        ...
        for (const pos of Object.keys(positions)) {
            ...
            for (let i = 0; i < posPlayers.length; i++) {
                ...
                playerPositions.set(player.Player, {
                    u, v,
                    size: (60 + (conColor / maxConColors[pos]) * 60) / textureCanvas.width,
                    player,
                    color,
                    longitude: centerLongitude,
                    longitudeRange: {
...
function updateSphereTexture() {
    ...
    if (squadPicture) {
        ...
        img.onload = () => {
            const radius = (10 / 180) * canvas.height; // 从北极到北纬60度的距离

            // 绘制图片，覆盖整个北极区域
            ctx.drawImage(img, 0, 0, canvas.width, radius * 2);
            ctx.restore();
    ...
    if (intersects.length > 0) {
        ...
        for (const [name, data] of playerPositions) {
            ...
            if (data.longitudeRange.end < data.longitudeRange.start) {
                // 处理跨越0度经线的情况
                if (longitude >= data.longitudeRange.start || longitude <= data.longitudeRange.end) {
                    distance = 0;
                } else {
                    distance = Math.min(
